...
static ngx_int_t ngx_http_mytest_handler(ngx_http_request_t *r)
{
    ...
    */
    ...
    ngx_str_t myhead_k = ngx_string("MyHead");
    ngx_str_t myhead_v = ngx_string("MyHeadValueSB");
    ...
    if (h == NULL)
        return NGX_ERROR;
    h->hash = 1;
    // h->key.data = myhead_k.data;
    // h->key.len = myhead_k.len;
    // h->value.data = myhead_v.data;
    // h->value.len = myhead_v.len;
    h->key = myhead_k;
    h->value = myhead_v;
